"""Reusable parallel parameter-sweep helper (multiprocessing across CPU cores).

Run many independent simulation cells (e.g. organism-lives across a parameter
grid) concurrently. The worker must be a top-level, importable function so it is
...
def run_sweep(
    ...
    worker returns (typically the input cell merged with its outputs).
    """
    cells = list(cells)
